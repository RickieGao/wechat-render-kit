---
name: new-theme
description: Use when the user wants to create their own 微信公众号 style theme — either a brand-new design from a brief, or one that continues your existing 公众号 look (DNA extraction). Triggers: "造个新主题", "我想要自己的风格", "/new-theme", "把我现有号的风格做成模板". Produces a new theme HTML + auto-registers it.
---
...
| 变量 | 说明 |
|---|---|
| `path`（路径） | `modern`（全新设计）或 `classic`（继承旧号风格） |
| `name`（主题名） | kebab-case 小写短横线，例 `mono-editorial`、`warm-ink` |
| `family` | `modern` 或 `classic`（通常与 `path` 一致） |
| `tone` | `humanist`（感性书页）或 `tech`（理性几何） |

**含糊时主动问，不猜**。典型需要追问的情形：

- 用户只说"造个主题"，没给名字 → 问：「主题名是什么（kebab-case）？基调是感性（humanist）还是理性（tech）？」
- 用户没说走哪条路径 → 问：「你是想从零设计一个全新风格（modern），还是想继承你现有公众号的视觉风格（classic）？」
- 名字含空格或中文 → 提示转换：「主题名需要是 kebab-case 格式，例如 `warm-ink`，你看用哪个？」

---

## 2. modern 路径：收集 brief

用户选择 modern 路径时，引导填写 `docs/wechat-render/references/theme-brief-template.md`。
...
**「必须覆盖」清单（全套 markdown 元素 + 7 个 primitives）已预填在模板里**，无需再问——直接告诉 subagent「按 design-brief-modern.md 的清单覆盖」即可。
...
**如果用户说没有旧文章 HTML**：建议改走 modern 路径，说明「classic 路径需要你现有文章的 HTML 作为视觉锚点，没有的话 modern 路径会更顺畅——你只需要描述你想要的感觉」。不要强行继续。
...
设计时参考 docs/wechat-render/references/design-brief-modern.md 的业界范式和 primitives 清单。
覆盖全套 markdown 元素 + 7 个 primitives（详见 brief）。
返回 Quick-Reference 摘要 + 写入确认。不要输出完整 HTML。
```

**prompt 模板（classic 路径）**：
...
再参考 docs/wechat-render/references/design-brief-classic.md 精化。
覆盖全套 markdown 元素 + 7 个 primitives（详见 design-brief-classic.md）。
返回 Quick-Reference 摘要 + 写入确认。不要输出完整 HTML。
```
...
用它渲染一篇文章试试效果：
  『用 <name> 渲染 <你的文章.md>』
（本仓库自带 examples/sample-article/article.md 可直接试；装到你自己项目后换成你的文章路径）
```
