...
- **DO render the article title as an `<h1>` at the top of the `<section>`, and the `summary` as the lede — both intentionally stay in the pasted body, and must remain selectable (NOT `user-select:none`; only `.preview-label` gets that).** The author **cuts** the title into WeChat's title field and the lede into the 摘要 field, entirely inside the editor — no switching back to the md / Notion to re-copy the title. (Author preference 2026-06-12: paste everything, then cut in-editor — easier than cross-interface copying the title; supersedes the earlier "don't render H1" rule.)
